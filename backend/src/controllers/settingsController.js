...
import { sendEmail, HTMLMail } from "../services/mailService.js"; // Correo promocional
...
settingsController.updateMarquee = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: "Marquee updated", marquee: updated.marquee }); // todo bien
    } catch (error) {
        console.log("error " + error);
        res.status(500).json({ message: "Internal server error" }); // Error del servidor
    }
...
settingsController.updateEmail = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: "Email updated and sent", email: updated.email }); // todo bien
  } catch (error) {
    console.log("error " + error);
    res.status(500).json({ message: "Internal server error" }); // Error del servidor
  }
