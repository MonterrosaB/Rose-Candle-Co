...
import { config } from "../config.js"; // Opcional, si quieres guardar la API key ahí

// API key de Brevo
const apiKey = config.brevo.brevoapi;

// Función para enviar correo de confirmación con Brevo
const sendAccountConfirmationEmail = async (to, code, name = "Usuario") => {
  try {
    // Contenido HTML
    const htmlContent = `
      <div style="font-family: 'Segoe UI', Helvetica, Arial, sans-serif; background-color: #F9F7F3; padding: 0; max-width: 600px; margin: 0 auto; border-radius: 20px; overflow: hidden; border: 1px solid #DFCCAC; box-shadow: 0 8px 24px rgba(0, 0, 0, 0.06);">
        ...
        <div style="background-color: #FFFFFF; padding: 40px 30px; position: relative; text-align: center;">
          <h1 style="color: #A78A5E; font-size: 22px; font-weight: 650; margin-bottom: 12px;">Confirma tu cuenta</h1>
          <p style="color: #6F6A50; font-size: 15px; line-height: 1.6; margin-bottom: 20px;">
            ¡Hola, <strong>${name}</strong>!<br>
            Para activar tu cuenta en Rosé Candle Co., utiliza el siguiente código de verificación:
          </p>
          ...
          <p style="color: #8E8A76; font-size: 13.5px; line-height: 1.5; margin-top: 30px;">
            Este código expirará en <strong>20 minutos</strong>. Si no solicitaste esta acción, puedes ignorar este correo.
          </p>
    ...
    const response = await fetch("https://api.brevo.com/v3/smtp/email", {
      ...
      body: JSON.stringify({
        ...
        to: [{ email: to, name }],
        subject: "Confirma tu cuenta - Rosé Candle Co.",
        htmlContent,
      }),
    });
