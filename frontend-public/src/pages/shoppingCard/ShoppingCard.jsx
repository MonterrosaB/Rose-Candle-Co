          ...
          cartItems.map((item, idx) => (
            ...
            >
              ...
              <div className="flex flex-col flex-grow">
                <h3 className="text-lg font-semibold text-[#444]">{item.name}</h3>
                <p className="text-[#666] mt-1 text-sm">${parseFloat(item.currentPrice || 0).toFixed(2)}</p>
                <div className="text-xs text-gray-500 mt-1">
                  <p>* Caja de regalo</p>
                  <p>* Tarjeta personalizada</p>
                </div>
                <div className="mt-2 flex gap-2">
                  <span className="bg-[#e6e6e6] text-xs px-2 py-1 rounded">8oz</span>
                  <span className="bg-[#e6e6e6] text-xs px-2 py-1 rounded">Vaso</span>
                </div>
              </div>
              <motion.button
  ...
  <div className="space-y-3">
    {cartItems.map((item, idx) => (
      ...
      >
        ...
        <span className="font-medium text-gray-800">${parseFloat(item.currentPrice || 0).toFixed(2)}</span>
      </motion.div>
    ))}
  </div>
